        ...
        return request(myRequest, function (error, response) {
			...
			describe('Update External Order Number', function () {
				...
				it('should update the external order number', function () {					
					var options = [{ 'c_externalOrderNo': 'glovia_001' }];
					...
					return request(myRequest, function (error, response) {
						assert.equal(response.statusCode, 200, 'Update External Order number call should return statusCode of 200');
						assert.containSubset(bodyAsJson.order_no, expectedTokenJSON.order_no, 'Actual response. Order number must match.');
						assert.containSubset(bodyAsJson.site_id, expectedTokenJSON.site_id, 'Actual response. Site Id must match.');
						assert.containSubset(bodyAsJson._type, expectedTokenJSON._type, 'Actual response. type must be order.');
					});
				});
			});
